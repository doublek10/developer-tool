...
import WebCloner from "./views/WebCloner";
...
type ViewId =
  | "dashboard" | "cv" | "pdf" | "clone" | "website" | "servers" | "analyzer" | "editor"
...
const NAV: NavItem[] = [
  ...
  { id: "clone", label: "Web page clone", group: "Create", needsNet: true },
...
export default function App() {
  ...
  const [editorRoot, setEditorRoot] = useState<string>("");
  ...
  const openInEditor = (path: string, root?: string) => {
    if (root) setEditorRoot(root);
    setEditorFile(path);
    setView("editor");
  };
      ...
      <div className="flex-1 flex min-h-0">
        ...
        <main className="flex-1 min-w-0 flex flex-col">
          ...
          <div className="flex-1 min-h-0 overflow-auto p-4">
            ...
            {view === "cv" && <CvBuilder />}
            {view === "pdf" && <PdfStudio />}
            {view === "clone" && <WebCloner online={online} onOpenInEditor={openInEditor} />}
            ...
            {view === "editor" && (
              <Suspense fallback={<Busy label="Loading editor…" />}>
                <CodeEditor initialPath={editorFile} initialRoot={editorRoot} />
              </Suspense>
            )}
