...
export default function SignIn({
  onSignedIn,
}: { onSignedIn: (u: SessionUser) => void; theme: string }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const user = await call<SessionUser>("sign_in", { username, password });
      onSignedIn(user);
    } catch (err) {
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <form onSubmit={submit} className="w-full max-w-[320px]">
          ...
          <p className="text-[12.5px] text-muted mt-1 mb-6">
            This account exists only on this computer.
          </p>
          ...
          <p className="text-[11.5px] text-muted mt-5 leading-relaxed">
            Forgot the password? There is no reset — the hash never leaves this
            machine. Delete the <span className="mono">database</span> folder in
            the DevWorkstation data directory to start over.
          </p>
        </form>
      </div>
    </div>
