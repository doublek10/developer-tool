...
**Windows trust warnings.** Unsigned, so Edge's download check ("isn't
commonly downloaded") and SmartScreen's launch warning ("Unknown publisher")
both show up — harmless, click through them, but they cost you users. Fixing
this for good needs one of:

- A real Authenticode code-signing certificate (OV/EV from DigiCert, Sectigo,
  SSL.com, etc. — costs money) or Azure Trusted Signing (a cheaper Microsoft
  subscription). Either way, signing happens with `signtool.exe` against the
  built `.exe`, not through `TAURI_SIGNING_PRIVATE_KEY` — that secret only
  signs Tauri's *auto-updater* packages (spec §28) and has no effect on this
  warning.
- Packaging as MSIX and distributing through the Microsoft Store (one-time
  developer account fee, ~$19 individual / ~$99 company) — Store-installed
  apps skip both warnings entirely.

**Free things that help in the meantime**, while there's no signing budget:
the build workflow now publishes tagged releases (`v0.1.0`, etc.) as GitHub
Releases rather than only a workflow artifact — downloads from an
established domain like github.com carry more reputation with Edge/
SmartScreen than a brand-new one, which measurably reduces (never
eliminates) the banner. `bundle.publisher` and `bundle.copyright` are also
now set in `tauri.conf.json`, since a blank publisher field is one of the
signals both checks weigh. None of this removes the warning for certain —
only signing or the Store does that.
