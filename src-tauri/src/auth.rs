...
pub async fn register(
    ...
    let (valid, status, message) = crate::license::verify_remote(&activation_key).await.map_err(|e| {
        AppError::new(
            "NO_INTERNET",
    ...
    if !valid {
        let conn = db.0.lock().unwrap();
        logging::write(
            &conn, "warn", "AUTH", "Registration rejected", Some(&format!("status={status}")),
        );
        return Err(AppError::new("ACTIVATION_INVALID", message)
...
pub async fn sign_in(
    ...
    if crate::system::is_online() {
        ...
        if let Some(key) = key {
            if let Ok((valid, status, message)) = crate::license::verify_remote(&key).await {
                let conn = db.0.lock().unwrap();
                let _ = crate::license::save_license(&conn, &key, &status);
                if !valid {
                    logging::write(
                        &conn, "warn", "AUTH", "Sign-in blocked",
